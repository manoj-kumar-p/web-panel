...
class Bank extends Component {
  ...
  handleSubmit = (e) => {
    ...
    if (account_name && account_number && bank_name) {
      ...
      axios
        .put(url + "/bank/" + bank_id, data, headers(this.props.user.token))
        .then((res) => {
          this.setState({
        ...
        {edit ? (
          ...
          <div className="bank">
            <div className="action">
              <span onClick={() => this.setState({ action: !action })}>
                ...
              </span>
              {action && (
                <div>
                  <button
                    onClick={() => this.deleteBank(bank.bank_id)}
                    style={{ backgroundColor: "#ED515E" }}
                  >
                    Delete
                  </button>
                  <button
                    onClick={() => this.setState({ edit: true })}
                    style={{ backgroundColor: "#4694FC", top: "50px" }}
                  >
                    Edit
                  </button>
                </div>
